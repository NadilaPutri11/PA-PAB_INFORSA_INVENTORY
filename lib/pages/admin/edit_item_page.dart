...
class _EditItemPageState extends State<EditItemPage> {
  ...
  late final TextEditingController _kodeBarangController;
  late final TextEditingController _namaBarangController;
  late final TextEditingController _volumeController;
  late final TextEditingController _spesifikasiController;
  ...
  late DateTime _tanggalPembukuan;
  late String _asalBarang;
  late String _satuan;
  ...
  void initState() {
    ...
    _kodeBarangController = TextEditingController(text: item.kodeBarang);
    _namaBarangController = TextEditingController(text: item.namaBarang);
    _volumeController = TextEditingController(text: item.volume.toString());
    _spesifikasiController = TextEditingController(
      text: item.spesifikasiBarang ?? '',
    );
    _tahunController = TextEditingController(
      text: item.tahunPembuatan?.toString() ?? '',
    );
    _hargaController = TextEditingController(
      text: item.hargaBarang != null
          ? NumberFormat('#,###', 'id_ID').format(item.hargaBarang)
          : '',
    );
    _keteranganController = TextEditingController(
      text: item.keteranganTambahan ?? '',
    );
    _tanggalPembukuan = item.tanggalPembukuan ?? DateTime.now();
    _tanggalController = TextEditingController(
      text: DateFormat('dd/MM/yyyy').format(_tanggalPembukuan),
    );

    _asalBarang = item.asalBarang;
    _satuan = item.satuan;
    _kondisi = item.kondisiBarang;
  ...
  Future<void> _handleSimpan() async {
    ...
    String? dokumenUrl = widget.item.dokumenNotaUrl;
    ...
    // Hanya ambil tahun & harga jika "Beli"
    ...
    final updatedItem = ItemModel(
      ...
      volume: vol,
      tersedia: widget.item.tersedia, // FIX: Preserve existing available count
      satuan: _satuan,
    ...
    if (success) {
      ...
      _showSnackBar('Gagal memperbarui barang', isError: true);
    }
  }
  ...
  Future<void> _handleHapus() async {
    final confirm = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          'Yakin ingin menghapus "${widget.item.namaBarang}"?\nTindakan ini tidak dapat dibatalkan.',
        ),
        actions: [
          TextButton(
    ...
    if (success) {
      ...
      _showSnackBar('Gagal menghapus barang', isError: true);
    }
  }
      ...
      appBar: AppBar(
        ...
        actions: [
          IconButton(
            ...
            onPressed: isProcessing ? null : _handleHapus,
          ),
        ],
      ),
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              const Text(
                'Perbarui informasi aset yang sudah tercatat dalam inventaris.',
                ...
                child: Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(
                      color:
                          (_fotoBytes != null ||
                              (_existingFotoUrl != null && !_hapusFoto))
                          ? navyColor
                          : Colors.transparent,
                      width: 2,
                    ),
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      isPdf ? 'Dokumen PDF baru' : 'Foto Nota baru',
